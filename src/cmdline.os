...
Функция РазобратьАргументы(Знач ИменованныеПараметры, Знач ПозиционныеПараметры)
	...
	Пока Истина Цикл
		...
		ИначеЕсли СтрНайти(Токен, "=") > 0 И Лев(Токен, 2) = "--" Тогда
			...
			ЗначениеТокена = Сред(Токен, Индекс + 1);
			Если ЭтоИменованныйПараметр(СокрЛП(КлючТокена), ИменованныеПараметры) Тогда
				Лог.Отладка("Это именованный параметр: " + КлючТокена + " ?");
				СтрПараметр = ИменованныеПараметры.Найти(КлючТокена, "Имя");
				Если СтрПараметр.ЭтоКоллекция Тогда
					Лог.Отладка("Добавляем элемент в коллекцию");
					ТекКолекция = РезультатРазбора[КлючТокена];
					Если ТекКолекция = Неопределено Тогда
						ТекКолекция = Новый Массив;
						РезультатРазбора[КлючТокена] = ТекКолекция;
					КонецЕсли;
					ТекКолекция.Добавить(ЗначениеТокена);
				Иначе
					РезультатРазбора[КлючТокена] = ЗначениеТокена;
				КонецЕсли;
				Лог.Отладка("Нашли значение именованного параметра: " + РезультатРазбора[КлючТокена]);
			Иначе
